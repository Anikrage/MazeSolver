...
class MazeEnvironment:
    def __init__(self, maze):
        ...
        # Ensure the start and end positions are valid (path, not wall)
        assert self.maze[self.start_pos] == 0, "Start position must be a path (0), not a wall (1)"
        assert self.maze[self.goal_pos] == 0, "Goal position must be a path (0), not a wall (1)"
    ...
    def step(self, action):
        ...
        Returns:
            A tuple (new_state, reward, done, info) where:
                - new_state is the new position
                - reward is the reward for this step
                - done is True if the episode is over
    ...
    def get_state_space_size(self):
        """
        Returns the size of the state space (number of possible positions).
        """
